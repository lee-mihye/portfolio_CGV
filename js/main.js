...
// 세미콜론 안하는 것들!....
//1.함수 2.제어문(id, for, while, switch case 등..)


/*////////////////////////////////////////////////////////////
    함수명: show
    기능: 미리 셋팅된 class를 제작노트 박스에 넣고 뺌
*/ ////////////////////////////////////////////////////////////
function show(seq) {
    //seq - 전달값 변수(공통순번)
    //1. 호출확인!
    ...
    //2. 변경대상 : .pnt(제작노트박스)
    var tg =
        document.getElementsByClassName("pnt").item(seq);
    
    // 여기서 내용에 따라 높이값이 달라지므로 높이값을 별도 세팅함!
    // 높이값을 읽어와라!(높이값이0인 p태그 내부의 span태그의 높이값)
    var tgh = tg.getElementsByTagName("span")[0].offsetHeight;
    //offsetHeight는 선택요소의 높이값 읽기(패딩, 보더 포함)
    //clientHeight는 선택요소의 높이값 일기(패딩만 포함)
    console.log("높이값:"+tgh);
    
    //3. 변경내용 : 기존 class에 "on"을 추가한다!
    ...
    // "ON"이 클레스에 있으면 if문 안으로 들어감!
    if (tg.classList.contains("on")) {
        sbtn.innerText = "－";
        
        //높이값이 다르므로 별도설정!(특정높이 만들기)
        tg.style.height = tgh + "px";
    } ////////////////// if 끝 ////////////////////////////
    else { //"ON"이 클레스에 없는 경우
        sbtn.innerText = "＋";
        
        //높이값이 다르므로 별도설정!(0만들기)
        tg.style.height = tgh + "0px";
    } ////////////////// else 끝 /////////////////////////
